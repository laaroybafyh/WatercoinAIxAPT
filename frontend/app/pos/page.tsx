...
import { saveSnapshotToCanister, addSurvey, idrToAptos } from '../../lib/aptosAgent';
import AptosPayment from '../../components/AptosPayment';
import type { PaymentTransaction } from '../../lib/aptosAgent';

type Stage = 'items' | 'payment' | 'aptos-payment' | 'result' | 'dispense' | 'survey';
...
export default function POSPage() {
  ...
  const [aptosTransaction, setAptosTransaction] = useState<PaymentTransaction | null>(null);
    ...
    <div className={styles.wrapper}>
      ...
      {stage === 'items' && (
        <section className={styles.items}>
          {products.map(p => {
            const aptosAmount = Math.ceil(idrToAptos(p.price) * 100) / 100; // Round up to 2 decimal places
            return (
        ...
        <section>
          ...
          <p style={{color: '#FFFFFF', marginBottom: '24px', textAlign: 'center'}}>
            ...
            <span style={{fontSize: '0.9rem', color: '#FFFFFF', fontWeight: '600'}}>
              ~ {(Math.ceil(idrToAptos(product.price) * 100) / 100).toFixed(2)} APTOS
            </span>
          </p>
          <div className={styles.items}>
            {/* APTOS Payment - Updated with standard white box style */}
            <button 
              className={styles.item} 
              onClick={() => setStage('aptos-payment')}
            >
              <div className={styles.itemName}>Pay with APTOS</div>
              <div style={{color: '#FFFFFF', fontSize: '0.9rem', marginTop: '4px', fontWeight: '600'}}>
                Blockchain Payment
              </div>
              <div className={styles.brandLogo}>
                <img src="/aptos.png" alt="APTOS" className={styles.logoImg} />
              </div>
            </button>
      ...
      {stage === 'aptos-payment' && product && (
        <section className={styles.aptosPaymentSection}>
          <AptosPayment
            productId={product.id}
            productName={product.name}
            priceIDR={product.price}
            onSuccess={(transaction) => {
              setAptosTransaction(transaction);
              setPaid(true);
              setStage('result');
            }}
            onCancelAction={() => setStage('payment')}
            onError={(error) => {
              console.error('APTOS payment error:', error);
              setPaid(false);
              setStage('result');
            }}
          />
                ...
                {aptosTransaction && (
                  <div style={{background: '#f0f9ff', padding: '16px', borderRadius: '12px', marginBottom: '16px'}}>
                    <p style={{color: '#0D6CA3', fontSize: '1rem', margin: '4px 0'}}>
                      <strong>APTOS Transaction:</strong>
                    </p>
                    <p style={{color: '#666', fontSize: '0.9rem', margin: '4px 0'}}>
                      Amount: {aptosTransaction.aptosAmount.toFixed(6)} APT
                    </p>
                    <p style={{color: '#666', fontSize: '0.9rem', margin: '4px 0'}}>
                      IDR: Rp {aptosTransaction.idrAmount.toLocaleString('id-ID')}
                    </p>
                    {aptosTransaction.transactionHash && (
                      <p style={{color: '#666', fontSize: '0.8rem', margin: '4px 0', fontFamily: 'monospace'}}>
                        Hash: {aptosTransaction.transactionHash.slice(0, 16)}...
                      </p>
                    )}
                  </div>
                )}
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '32px', justifyContent: 'center', maxWidth: '700px', margin: '0 auto'}}>
            <button 
              ...
              onClick={async () => {
                ...
                try { await addSurvey('positive'); } catch(e){ console.warn('survey save failed', e); }
            ...
            <button 
              ...
              onClick={async () => {
                ...
                try { await addSurvey('neutral'); } catch(e){ console.warn('survey save failed', e); }
                setTimeout(() => window.location.assign('/'), 400);
              }}
            >
              <span style={{fontSize: '2rem', marginBottom: '8px'}}>😐</span>
            ...
            <button 
              ...
              onClick={async () => {
                ...
                try { await addSurvey('negative'); } catch(e){ console.warn('survey save failed', e); }
                setTimeout(() => window.location.assign('/'), 400);
              }}
            >
              <span style={{fontSize: '2rem', marginBottom: '8px'}}>😞</span>
